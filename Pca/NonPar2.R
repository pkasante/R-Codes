# Non Parametric Screening Using KS test and Wilcoxon test
vnames <- names(train)
dat<-train
...
for (j in cols.x){
  ...
  pvalue.equal.var <- pvalue.kstest <- logworth.kstest <- pvalue.wilcoxon <- logworth.wilcoxon <- NA
  ...
  if (n.marker >= 4) {
    ...
    #Kolmogorov Test
    pvalue.kstest <- ks.test(xj[prostate.cancer ==0],xj[prostate.cancer==1], alternative="two.sided", var.equal=equal.var)$p.value
    logworth.kstest <- -log10(pvalue.kstest)
    
    # WILCOXON RANK-SUM TEST
    pvalue.wilcoxon <- wilcox.test(xj~prostate.cancer, alternative="two.sided")$p.value
    logworth.wilcoxon <- -log10(pvalue.wilcoxon )
  }
  out <- c(n.marker, ybar.1, ybar.0, diff.mean, pvalue.equal.var, pvalue.kstest, 
           logworth.kstest, pvalue.wilcoxon, logworth.wilcoxon)  
  OUT <- rbind(OUT, out)
}
...
colnames(OUT) <- c("Biomarker", "n.marker", "ybar.1", "ybar.0", "diff.mean", "pvalue.equal.var", 
                   "pvalue.kstest", "logworth.kstest", "pvalue.wilcoxon", "logworth.wilcoxon")
head(OUT)
...
variables.selected<-OUT1[min(OUT1$pvalue.equal.var,OUT1$pvalue.kstest,OUT1$pvalue.wilcoxon,1.0,na.rm = TRUE)<=0.05/3 &OUT1$n.marker>=5,1]
